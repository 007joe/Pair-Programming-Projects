# Module 2 - Week 2 Review: National Park Campsite

In this exercise, you'll be working with a National Park Campsite Reservation database. You'll create DAO methods and integration tests.
...
In the `/database` folder, there's a `campground.sql` SQL script that you need to use to create the `campground` database. In 'pgAdmin' you will need to create a new database. Then open the query tool for the database you just created. Then run the `campground.sql` script to install the tables and data you will use to reference while you work. Be aware, however, that the tests don't use that database. The tests use a temporary database with the same structure. You'll find the SQL for that temporary database in `src/test/resources/test-data.sql`.
